...
function toolTable() {
  return [
    ...
    T(
      "page_eval",
      "在当前页面上下文执行一段 JavaScript 并返回结果（用于读取/计算页面内变量、验证猜想）。",
      {
        type: "object",
        properties: {
          expression: { type: "string", description: "要执行的 JS 表达式或 IIFE" },
    ...
    T(
      ...
      "在已捕获/落盘的 JS 语料中搜索字符串或正则，定位加密函数、参数名、常量等。",
      {
        ...
        properties: {
          ...
          regex: { type: "boolean", description: "query 是否按正则处理，默认 false" },
    ...
    T(
      "signer_trace",
      "抓某个 JS 函数被调用时的**真实入参**（引擎层 Debugger 观测：旁观页面函数调用读 frame.arguments，**不注入页面、不改页面全局**，满足只读红线）。" +
    ...
    T(
      ...
      "离线拆解 JSVMP dispatcher：dispatcher 函数源码 → handlers.json（decode_table + 每个 opcode 语义）。内部 run_node 跑随浏览器打包的离线工具（babel 已内联，无需 node_modules）。需先设工作目录。",
      {
        type: "object",
        properties: {
    ...
    T(
      "recall",
      "检索**跨会话/跨站点**的历史记忆库（remember 沉淀的已确认事实/已否决死路，SQLite 持久化）。" +
        "开工前或换方向时查一下：以前在**这个站点或类似目标**上确认过什么、排除过哪些死路，省得重新发现/重走。" +
        "当前站点的记忆每轮已自动注入到你上下文；recall 用于**翻其它站点/更早会话**或按关键词精确查。",
      {
        type: "object",
        properties: {
          query: { type: "string", description: "关键词子串（匹配事实正文，如 签名函数名 / 某指纹项 / nonce / 目标参数名）" },
          site: { type: "string", description: "站点域名子串过滤（省略=全部站点）" },
