...
export const agentSession = {
   ...
   */
  async run(threadId, { systemPrompt, convo, confirmMode = false, maxRounds = 80, maxPerTool = 40, workspaceRoot, win, assist = false } = {}) {
    ...
    try {
      ...
      const MAX_AUTO_RESTARTS = 8; // 一次提问最多自动续 8 个 turn（≈640 轮），硬上限防无限空转
      let turnMsgs = convo;
      let autoRestarts = 0;
      let driftStreak = 0;
      let res;
      ...
      turnMsgs.push({
        role: "user",
        content:
          "（系统·自动续跑）上一段到达轮次/调用上限但任务还没完成。基于已落盘进展（progress.md/工作目录文件）" +
          "继续推进到底，别从头重来、别重复已确认过的发现；只有给出可独立实跑的产物、或真需要我提供你拿不到的" +
          "东西（登录态/账号/验证码/纯业务决策）时才停。",
      });
      } // end for(;;) —— 全自动续跑
